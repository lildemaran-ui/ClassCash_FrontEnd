import { ImagePlus, Loader, Receipt, Send, ShieldAlert, Wallet } from "lucide-react";
  ...
  return (
    <div className="flex flex-col gap-4 p-5 bg-white border border-gray-200 rounded-2xl shadow-lg animate-in slide-in-from-bottom-4 w-full max-w-full">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-xs text-amber-700">
        <Loader /> O seu pagamento será validado pela Secretaria. Guarde o ID de transação.
      </div>
      ...
      <button
        ...
        className={`w-full py-4 rounded-xl font-bold text-base flex items-center justify-center gap-2 transition-all active:scale-[0.985] shadow-md ${
          loading
            ...
            : "bg-[#184d8a] text-white hover:bg-[#184d8a]/90"
        }`}
      >
        {loading ? (
          <>
...
export default function PagamentoGeral() {
 ...
 const [mostrarComprovativo, setMostrarComprovativo] = useState(false);
  ...
  const iframeUrl =
    token && user?.codigoPlataforma && iban && iban !== "A carregar..." && iban !== "Não disponível"
...
// Escutar resposta da Multicaixa Express
  useEffect(() => {
    ...
    if (!event.origin.includes("v0-multicaixa-express-app.vercel.app") && 
...
console.log("📨 PostMessage recebido:", event.data); // ← Mantém isto para debug
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
   ...
   if (name === "metodo") {
    ...
    if (name === "servicoId") {
      setPagamento((prev) => ({ ...prev, mesInicial: "Janeiro", mesFinal: "Janeiro" }));
    }
  };
   ...
   if (pagamento.metodo === "De forma digital") {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setImage(null);
        setImageFile(null);
        setPagamento((p) => ({ ...p, metodo: "", plataforma: "", servicoId: servicosDisponiveis[0] ? String(servicosDisponiveis[0].idservico) : "" }));
      } else {
    ...
    try {
      ...
      if (isEncarregado) {
        ...
        fetch(`http://localhost:5000/api/estudante/${idusuario}`)
          .then((r) => r.json())
          .then((dados) => {
            setUser({ ...userBase, codigoPlataforma: dados.codigo_plataforma ?? undefined });
          })
          ...
          <div className="max-w-2xl mx-auto py-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
              ...
              <div className="flex flex-col gap-6">
                ...
                {perfil === "encarregado" && (
                  <div>
                    <label className="block mb-1 font-bold text-gray-700">Educando</label>
                    <input
                      type="text"
                ...
                {pagamento.metodo !== "" && pagamento.metodo !== "Dinheiro Físico" && (
                  <div className="flex flex-col gap-4 animate-in fade-in">
                    ...
                    {isPropina && !mostrarComprovativo && (
                      <div className="flex flex-col gap-3 p-4 bg-blue-50 border border-blue-100 rounded-xl animate-in fade-in">
                        ...
                        {indexFim < indexInicio && (
                          <p className="text-xs text-red-500">⚠️ O mês final deve ser igual ou posterior ao mês inicial.</p>
                        )}
                        <p className="text-xs text-blue-600 font-semibold">{quantidadeMeses} mês(es) selecionado(s)</p>
                      </div>
                    )}
                    ...
                    {pagamento.metodo === "De forma digital" && !mostrarComprovativo && (
                      <div>
                        <label className="font-medium block mb-2 text-gray-700">Plataforma Digital:</label>
                    ...
                    {!mostrarComprovativo && (
                      <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                        <label className="block text-xs text-blue-500 font-semibold mb-1">Total a pagar</label>
                        <p className="text-2xl font-black text-blue-700">KZ {valorTotal.toLocaleString("pt-PT")},00</p>
    ...
    <p className="text-center text-xs text-gray-500">
      Se o comprovativo não apareceu automaticamente após pagar
                ...
                {pagamento.metodo === "No banco" && !mostrarComprovativo && (
                  <div className="flex flex-col gap-4 animate-in fade-in">
                    <h3 className="font-bold text-gray-800 border-b pb-2">Dados para transferência</h3>
                    <div>
                    ...
                    <div>
                      <label className="font-medium block mb-2 text-gray-700">Comprovativo de depósito</label>
                      <label htmlFor="fotoInput"
                        ...
                        {image ? (
                          <img loading="lazy" src={image} alt="preview" className="h-28 object-contain rounded-lg" />
                        ) : (
                          <>
                  ...
                  <button
                    ...
                    className={`py-4 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all active:scale-95 ${
                      imageFile && pagamento.servicoId
                        ? "bg-[#184d8a] text-white hover:bg-[#184d8a]/85 shadow-md"
                        : "bg-gray-200 text-gray-400 cursor-not-allowed"
                    }`}
                  >
                    <Receipt size={18} />
      ...
      <div className="hidden lg:flex w-1/2 items-center justify-center bg-gray-50 p-8 border-l">
        <img src={img1} alt="Credit Card" className="w-full max-w-sm opacity-80" />
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl p-6 md:p-8 max-w-md w-full text-center shadow-2xl">
            ...
            <button
              onClick={() => { setShowModal(false); setPagamento((p) => ({ ...p, metodo: "" })); }}

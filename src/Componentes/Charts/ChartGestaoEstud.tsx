...
const survivalData: ChartDatum[] = [
  {
    id: 'Ativos',
    label: 'Ativos:',
    value: titanicData.filter(d => d.Survived === 'Ativos').reduce((sum, d) => sum + d.Count, 0),
    percentage: (titanicData.filter(d => d.Survived === 'Ativos').reduce((sum, d) => sum + d.Count, 0) / totalCount) * 100,
    color: '#268cffc5', // Cor para Sobreviventes
  },
  {
    id: 'Inativos',
    label: 'Inativos:',
    value: titanicData.filter(d => d.Survived === 'Inativos').reduce((sum, d) => sum + d.Count, 0),
    percentage: (titanicData.filter(d => d.Survived === 'Inativos').reduce((sum, d) => sum + d.Count, 0) / totalCount) * 100,
    color: '#268cff', // Cor para Não Sobreviventes
  },
];
...
export default function TitanicSurvivalPie() {
  ...
  return (
     ...
     <Box sx={{ width: 'auto', textAlign: 'center', py: 4 , transition: 'all 50s ease-in-out'}}>

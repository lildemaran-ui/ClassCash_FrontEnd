...
import {
  Download, RefreshCcw, AlertTriangle, Users, FileText,
  X, CheckCircle, TrendingUp, TrendingDown, BarChart2
} from "lucide-react";
import { useEffect, useState } from "react";
import { toast } from "sonner";

/* ── Modal de Exportação ── */
const ModalExportar = ({ titulo, onClose }: { titulo: string; onClose: () => void }) => {
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="bg-blue-50 rounded-xl p-4 border border-blue-100">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-xs font-semibold text-blue-700">Pronto para exportar</p>
            </div>
            <p className="text-xs text-blue-600">
              {titulo} · Período: {periodo === "mes" ? "Este Mês" : periodo === "trimestre" ? "1º Trimestre" : "Ano Lectivo 2024"} · Formato: {formato.toUpperCase()}
            </p>
          </div>
        </div>
...
const RELATORIOS = [
  {
    ...
    bgIcon: "bg-blue-50 text-[#184d8a] group-hover:bg-[#184d8a] group-hover:text-white",
  ...
  },
  {
    id: "alunos",
    icon: <Users size={20} />,
    bgIcon: "bg-orange-50 text-orange-500 group-hover:bg-orange-500 group-hover:text-white",
  ...
  },
  {
    ...
    bgIcon: "bg-red-50 text-red-500 group-hover:bg-red-500 group-hover:text-white",
    ...
    <div className="flex items-end gap-2 h-full">
      {meses.map((m, i) => (
        <div key={m} className="flex-1 flex flex-col items-center gap-1">
          <div
            className="w-full rounded-t-md bg-[#184d8a]/20 relative overflow-hidden"
            style={{ height: `${(valores[i] / max) * 160}px` }}
          >
            <div
              className="absolute bottom-0 w-full bg-[#184d8a] rounded-t-md transition-all duration-700"
              style={{ height: "100%" }}
            />
          </div>
...
export default function Relatorio() {
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans overflow-hidden custom_scroll">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        />
          <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl p-2">
            <div className="flex flex-col px-2">
              <label className="text-[10px] font-bold text-gray-400">Período</label>
              <select
                value={periodo}
            ...
            <button className="bg-[#184d8a] text-white p-2 rounded-lg hover:bg-[#1a5fad] transition-all">
              <RefreshCcw size={16} />
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
            {[
              { label: "Receita Total", val: "12.4M AOA", icon: <TrendingUp size={14} />, color: "text-green-500", bg: "bg-green-50" },
              { label: "Alunos Activos", val: "248", icon: <Users size={14} />, color: "text-blue-500", bg: "bg-blue-50" },
              { label: "Taxa de Pagamento", val: "87%", icon: <BarChart2 size={14} />, color: "text-[#184d8a]", bg: "bg-blue-50" },
              { label: "Em Inadimplência", val: "7", icon: <TrendingDown size={14} />, color: "text-red-500", bg: "bg-red-50" },
            ].map(({ label, val, icon, color, bg }) => (
              <div key={label} className="bg-white rounded-2xl border border-gray-100 p-4 hover:shadow-md transition-all">
                <div className={`w-8 h-8 ${bg} ${color} rounded-xl flex items-center justify-center mb-3`}>{icon}</div>
          ...
          {/* Cards de Download */}
          <h3 className="font-bold text-gray-700 mb-4">Relatórios Disponíveis</h3>
            ...
            {RELATORIOS.map(rel => (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div className={`p-3 rounded-xl transition-all ${rel.bgIcon}`}>{rel.icon}</div>
                  <div className="flex items-center gap-2">
                    <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${rel.badge.cls}`}>{rel.badge.text}</span>
                    <Download size={15} className="text-gray-300 group-hover:text-gray-500 transition-all" />
                  </div>
                </div>
                <h4 className="font-bold text-gray-700 text-sm mb-1">{rel.titulo}</h4>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h3 className="font-bold text-gray-700">Crescimento de Receita</h3>
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
              <div className="flex justify-between items-center mb-6">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">Ano lectivo 2024</p>
                </div>
              </div>
              <div className="flex items-center gap-6">
                <div className="relative w-36 h-36 flex-shrink-0">
                  <svg viewBox="0 0 36 36" className="w-full h-full -rotate-90">
                    ...
                    <circle cx="18" cy="18" r="15.9" fill="transparent" stroke="#184d8a" strokeWidth="3"
                      strokeDasharray="75 25" strokeLinecap="round" />
                    <circle cx="18" cy="18" r="15.9" fill="transparent" stroke="#f97316" strokeWidth="3"
                      strokeDasharray="15 85" strokeDashoffset="-75" strokeLinecap="round" />
                    <circle cx="18" cy="18" r="15.9" fill="transparent" stroke="#ef4444" strokeWidth="3"
                      strokeDasharray="10 90" strokeDashoffset="-90" strokeLinecap="round" />
                  </svg>
                  <div className="absolute inset-0 flex items-center justify-center">
                    <p className="text-xs font-bold text-gray-400 text-center leading-tight">Total<br/>100%</p>
                  </div>
                </div>
                <div className="space-y-3 flex-1">
                  {[
                    { color: "bg-[#184d8a]", label: "Propinas", pct: "75%" },
      ...
      {modalExportar && (
        <ModalExportar titulo={modalExportar} onClose={() => setModalExportar(null)} />
      )}
    </div>
  );
}
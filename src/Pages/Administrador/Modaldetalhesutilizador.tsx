...
// ─── Modal principal ──────────────────────────────────────────────────────────
export default function ModalDetalhesUtilizador({
  utilizador,
  onClose,
  onUpdated,
}: Props) {
  ...
  const handleEnviarLink = async () => {
    ...
    try {
      ...
      const res = await fetchComAuth(
        `${API_BASE}/admin/enviar-link-recuperacao`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const perfilCor =
    PERFIL_COR[utilizador.perfil] ??
    "bg-gray-100 text-gray-600 border-gray-200";
  const statusCor =
    STATUS_COR[utilizador.status] ?? "bg-gray-100 text-gray-500";
        ...
        <div className="relative bg-gradient-to-br from-[#184d8a] to-[#1a6fd4] px-6 pt-6 pb-10">
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <div className="flex gap-2 mt-1">
              <span
                className={`text-xs font-semibold px-3 py-1 rounded-full border ${perfilCor}`}
              >
                {utilizador.perfil}
              </span>
              <span
                className={`text-xs font-semibold px-3 py-1 rounded-full ${statusCor}`}
              >
                {utilizador.status}
              </span>
            </div>
            ...
            {[
              {
                icon: Building2,
                label: "Instituição",
                value: utilizador.instituicao,
              },
              {
                icon: Calendar,
                label: "Último Acesso",
                value: utilizador.ultimoAcesso,
              },
              {
                icon: Shield,
                label: "Plano",
                value: utilizador.planoActivo ? "Activo" : "Inactivo",
                cor: utilizador.planoActivo
                  ? "text-green-600"
                  : "text-gray-400",
              },
              ...
              <div key={label} className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p
                    className={`text-xs font-semibold truncate ${cor ?? "text-gray-700"}`}
                  >
                    {value}
                  </p>
                </div>
              ...
              {etapa === "escolher" && (
                <div className="space-y-3 animate-in fade-in duration-200">
                  <p className="text-xs text-gray-500 text-center mb-3">
                    Escolhe como redefinir a senha de{" "}
                    <strong className="text-gray-700">{utilizador.nome}</strong>
                    :
                  </p>
                  ...
                  >
                    <div className="w-9 h-9 rounded-lg bg-blue-100 group-hover:bg-primary flex items-center justify-center shrink-0 transition-colors">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-gray-400 mt-0.5 leading-relaxed">
                        Cria uma senha temporária e partilha manualmente com o
                        utilizador.
                      </p>
                    </div>
                  </button>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-gray-400 mt-0.5 leading-relaxed">
                        Envia um link de recuperação para{" "}
                        <span className="text-[#184d8a] font-semibold">
                          {utilizador.email}
                        </span>
                      </p>
                    </div>
              ...
              {etapa === "gerar" && (
                <div className="space-y-3 animate-in fade-in duration-200">
                  <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-xs text-amber-700">
                    ⚠️ Isto vai <strong>substituir a senha actual</strong> do
                    utilizador. Partilha a nova senha de forma segura.
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      disabled={loading}
                      className="flex-1 py-2.5 rounded-xl bg-primary text-white text-xs font-bold flex items-center justify-center gap-2 hover:bg-primary/85 transition-all disabled:opacity-60 shadow-md"
                    >
                      {loading ? (
                        <>
                ...
                <div className="space-y-3 animate-in fade-in duration-200">
                  <div className="flex flex-col items-center gap-2 py-2">
                    ...
                    <p className="text-[11px] text-gray-400 text-center">
                      Partilha esta senha temporária com o utilizador de forma
                      segura.
                    </p>
                  </div>
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded-xl px-3 py-2 text-[11px] text-amber-700 flex items-start gap-2">
                    <Clock className="w-3.5 h-3.5 shrink-0 mt-0.5" />O
                    utilizador deve alterar esta senha após o primeiro login.
                  </div>

                  <button
                    onClick={onClose}
                    className="w-full py-2.5 rounded-xl bg-primary text-white text-xs font-bold hover:bg-primary/85 transition-all"
                  >
                    Fechar
                  </button>
                </div>

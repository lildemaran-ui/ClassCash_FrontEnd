...
const RECLAMACOES_MOCK: Reclamacao[] = [
  {
    id: 1,
    data: "22 Jan 2025",
    encarregado: "João Manuel",
    aluno: "Pedro Manuel",
    assunto: "Demora na entrega dos uniformes",
    status: "Pendente",
    descricao:
      "Os uniformes foram solicitados há mais de 3 semanas e até agora não foram entregues. Isso está a causar problemas para o aluno nas aulas de educação física.",
  },
  {
    id: 2,
    data: "19 Jan 2025",
    ...
    aluno: "Carlos Silva",
    assunto: "Erro no valor da propina",
    status: "Pendente",
    descricao:
      "Fui cobrado um valor superior ao acordado no início do ano lectivo.",
    resposta:
      "A situação está a ser analisada pelo departamento financeiro. Aguarde contacto.",
  },
  {
    id: 3,
    data: "15 Jan 2025",
    encarregado: "António Costa",
    aluno: "Ana Costa",
    assunto: "Pedido de justificativo de faltas",
    status: "Resolvida",
    descricao:
      "Necessito de justificativo para as faltas da semana de 8 a 12 de Janeiro.",
...
const ModalResposta = ({
  ...
  const handleSave = async () => {
    if (!resposta.trim()) {
      toast.error("Escreve uma resposta");
      return;
    }
    setSaving(true);
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div>
            ...
            <select
              value={status}
              onChange={(e) =>
                setStatus(e.target.value as Reclamacao["status"])
              }
  ...
  >
    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-6 text-center">
      ...
      <p className="text-gray-500 text-sm mb-6">
        Tens a certeza? Esta acção não pode ser revertida.
      </p>
...
export default function GestaodeReclamacoes() {
  const [reclamacoes, setReclamacoes] =
    useState<Reclamacao[]>(RECLAMACOES_MOCK);
  ...
  const handleUpdate = (
    ...
    setReclamacoes((prev) =>
      prev.map((r) => (r.id === id ? { ...r, resposta, status } : r)),
    );
  };
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
            ...
            <div className="bg-white p-5 rounded-2xl border-l-4 border-green-500 shadow-sm hover:shadow-md transition-all">
              ...
              <h3 className="text-3xl font-bold text-gray-800">
                {String(resolvidas).padStart(2, "0")}
              </h3>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-50">
                  {reclamacoes.map((r) => (
                    <tr
                      key={r.id}
                      className="hover:bg-primary/3 transition-colors text-center"
                    >

...
import {
  AlertTriangle, Bell, Building2, CalendarDays,
  CheckCircle,
  DollarSign,
  Edit2, Eye, EyeOff,
  Info,
  Loader2, Lock, Mail, MapPin, Phone,
  RefreshCw, Save, Settings, ShieldCheck, ShieldOff,
  Users, X,
} from "lucide-react";
...
const TABS: { id: TabId; label: string; labelShort: string; icon: React.ReactNode; desc: string }[] = [
  { id: "geral",        label: "Informações Gerais",  labelShort: "Geral",     icon: <Building2 size={16} />,    desc: "Dados da instituição" },
  { id: "pagamentos",   label: "Taxas e Multas",       labelShort: "Taxas",     icon: <DollarSign size={16} />,   desc: "Regras financeiras" },
  { id: "usuarios",     label: "Utilizadores",         labelShort: "Utilizad.", icon: <Users size={16} />,        desc: "Acessos e perfis" },
  { id: "seguranca",    label: "Segurança",            labelShort: "Segurança", icon: <Lock size={16} />,         desc: "Palavra-passe" },
  { id: "notificacoes", label: "Notificações",         labelShort: "Notif.",    icon: <Bell size={16} />,         desc: "Alertas e avisos" },
];
...
const Input = ({
  type = "text", placeholder, value, onChange, disabled = false, className = "",
}: {
  type?: string; placeholder?: string; value: string;
  onChange: (v: string) => void; disabled?: boolean; className?: string;
}) => (
  <input
...
const SaveBtn = ({
  onClick, loading, label = "Guardar Alterações", disabled = false,
}: {
  onClick: () => void; loading?: boolean; label?: string; disabled?: boolean;
}) => (
  <button
    onClick={onClick} disabled={loading || disabled}
    className="flex items-center justify-center gap-2 bg-[#184d8a] text-white px-5 sm:px-8 py-2.5 rounded-xl font-bold hover:bg-[#1a5fad] shadow-lg shadow-blue-200/40 transition-all text-xs sm:text-sm disabled:opacity-60 disabled:cursor-not-allowed"
  >
...
const Toggle = ({ checked, onChange }: { checked: boolean; onChange: () => void }) => (
  <button
    onClick={onChange}
    className={`w-12 h-6 rounded-full relative transition-all duration-300 ${checked ? "bg-[#184d8a]" : "bg-gray-300"}`}
  >
...
const TabGeral = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok) { const e = await res.json(); throw new Error(e.erro || "Erro ao guardar"); }
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      <SectionHeader
        ...
        action={
          <button onClick={carregar} className="p-2 text-gray-400 hover:text-[#184d8a] hover:bg-blue-50 rounded-xl transition-all" title="Recarregar dados">
            <RefreshCw size={14} />
          </button>
        }
      />
      ...
      <div className="bg-gradient-to-r from-[#184d8a]/5 to-blue-50/50 border border-[#184d8a]/10 rounded-2xl p-4 flex items-center gap-4 mb-2">
        <div className="w-12 h-12 bg-[#184d8a] rounded-2xl flex items-center justify-center shrink-0 shadow-md">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Nome da Escola" icon={<Building2 size={12} />}>
          <Input placeholder="Ex: Colégio Caracol" value={form.nome} onChange={(v) => setForm({ ...form, nome: v })} />
        </Field>

        <Field label="Ano Lectivo" icon={<CalendarDays size={12} />}>
          <Input placeholder="Ex: 2024/2025" value={form.ano_lectivo} onChange={(v) => setForm({ ...form, ano_lectivo: v })} />
        </Field>

        <Field label="Email de Contacto" icon={<Mail size={12} />}>
          <Input type="email" placeholder="secretaria@escola.ao" value={form.email} onChange={(v) => setForm({ ...form, email: v })} />
        </Field>

        <Field label="Telefone" icon={<Phone size={12} />}>
          <Input type="tel" placeholder="+244 900 000 000" value={form.telefone} onChange={(v) => setForm({ ...form, telefone: v })} />
        </Field>

        <div className="sm:col-span-2">
          <Field label="Endereço / Localização" icon={<MapPin size={12} />}>
            <Input placeholder="Rua, Bairro, Município, Província" value={form.endereco} onChange={(v) => setForm({ ...form, endereco: v })} />
          </Field>
        </div>
      </div>
...
const TabPagamentos = () => {
  const [form, setForm] = useState({
    percentualmulta: "10", diascarencia: "5", aplicarjuroscompostos: true,
  });
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="flex items-start gap-3 p-3 sm:p-4 bg-blue-50 rounded-2xl border border-blue-100">
        ...
        <p className="text-xs text-blue-700">
          As regras definidas aqui são aplicadas automaticamente a todas as propinas da instituição.
          Alterações entram em vigor no próximo ciclo de faturação.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Percentual de Multa (%)">
          ...
          />
          <p className="text-[11px] text-gray-400">Percentagem cobrada sobre o valor em atraso.</p>
        </Field>
        ...
        <Field label="Dias de Carência">
          ...
          />
          <p className="text-[11px] text-gray-400">Dias sem multa após a data de vencimento.</p>
        </Field>
      </div>
      ...
      <div className="flex items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-2xl border border-gray-200">
        <div>
          <p className="text-xs sm:text-sm font-bold text-gray-800">Juros compostos</p>
          <p className="text-[11px] sm:text-xs text-gray-400 mt-0.5">
            Juros calculados sobre o valor acumulado (incluindo multas anteriores).
          </p>
        </div>
        <Toggle
          checked={form.aplicarjuroscompostos}
          onChange={() => setForm({ ...form, aplicarjuroscompostos: !form.aplicarjuroscompostos })}
        />
      </div>
      ...
      <Field label="Moeda Padrão do Sistema">
        <div className="bg-gray-100 border border-gray-200 rounded-xl px-4 py-2.5 text-xs sm:text-sm text-gray-500 flex items-center gap-2 cursor-not-allowed">
          ...
          <span className="text-[11px] bg-[#184d8a]/10 text-[#184d8a] px-2 py-0.5 rounded-full font-bold ml-auto shrink-0">
            Padrão permanente
          </span>
        </div>
        <p className="text-[11px] text-gray-400">A moeda não pode ser alterada no sistema ClassCash.</p>
      </Field>
      ...
      {form.percentualmulta && form.diascarencia && (
        <div className="bg-[#184d8a]/5 rounded-2xl p-4 border border-[#184d8a]/10">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs font-bold text-[#184d8a]">Pré-visualização da Regra Activa</p>
          </div>
          <p className="text-xs sm:text-sm text-gray-600 leading-relaxed">
            Após <strong>{dias} {dias === 1 ? "dia" : "dias"}</strong> de atraso no pagamento,
            será aplicada uma multa de <strong>{multa}%</strong> sobre o valor em dívida,
            com juros <strong>{form.aplicarjuroscompostos ? "compostos" : "simples"}</strong>,
...
const ModalEditarUtilizador = ({
  ...
  user: Utilizador; onClose: () => void; onSave: (u: Utilizador) => void;
}) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok) { const e = await res.json(); throw new Error(e.erro || "Erro ao atualizar"); }
  ...
  return (
    ...
    >
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="bg-[#184d8a] px-5 sm:px-6 py-4 sm:py-5 flex justify-between items-center">
          <div>
            <h2 className="text-white font-bold text-sm sm:text-base">Editar Utilizador</h2>
            <p className="text-blue-200 text-xs">Altere os dados e permissões</p>
          </div>
          <button onClick={onClose} disabled={saving} className="p-2 rounded-xl bg-white/10 text-white hover:bg-white/20 transition-all disabled:opacity-40">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="px-5 sm:px-6 py-4 sm:py-5 space-y-3 sm:space-y-4">
          ...
          <div className="flex items-center gap-3 bg-gray-50 rounded-2xl p-3">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#184d8a]/10 text-[#184d8a] rounded-xl flex items-center justify-center font-bold text-base sm:text-lg shrink-0">
              {form.nome.charAt(0).toUpperCase()}
            </div>
            <div className="min-w-0">
              <p className="font-bold text-gray-700 text-xs sm:text-sm truncate">{form.nome || "—"}</p>
              <p className="text-[11px] sm:text-xs text-gray-400 truncate">{form.email || "—"}</p>
            </div>
          </div>

          <Field label="Nome Completo">
            <Input placeholder="Nome do utilizador" value={form.nome} onChange={(v) => setForm({ ...form, nome: v })} />
          </Field>

          <Field label="Email">
            <Input type="email" placeholder="email@escola.ao" value={form.email} onChange={(v) => setForm({ ...form, email: v })} />
          </Field>
          ...
          <Field label="Permissão / Papel">
            <div className="grid grid-cols-2 gap-2">
              {ROLES.map((r) => (
                <button
                  key={r} onClick={() => setForm({ ...form, role: r })}
                ...
                >
                  {form.role === r ? <ShieldCheck size={11} /> : <ShieldOff size={11} />}
                  {r}
                </button>
              ))}
            </div>
        ...
        <div className="px-5 sm:px-6 pb-5 sm:pb-6 flex gap-2 sm:gap-3">
          <button onClick={onClose} disabled={saving} className="flex-1 py-2.5 rounded-xl font-bold text-gray-500 border border-gray-200 hover:bg-gray-50 transition-all text-xs sm:text-sm disabled:opacity-40">
            Cancelar
          </button>
          <button
            onClick={handleSave} disabled={saving}
            className="flex-1 py-2.5 rounded-xl font-bold text-white bg-[#184d8a] hover:bg-[#1a5fad] transition-all shadow-md shadow-blue-200 flex items-center justify-center gap-2 text-xs sm:text-sm disabled:opacity-60"
          >
...
const TabUtilizadores = () => {
  ...
  const handleToggle = async (u: Utilizador) => {
    ...
    try {
      ...
      if (!res.ok) { const e = await res.json(); throw new Error(e.erro || "Erro ao alterar estado"); }
      ...
      <div className="space-y-4 sm:space-y-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <div className="bg-green-50 border border-green-100 rounded-2xl p-3 text-center">
            <p className="text-xl sm:text-2xl font-bold text-green-600">{ativos}</p>
            <p className="text-[11px] sm:text-xs text-green-600/70 font-medium mt-0.5">Activos</p>
          </div>
          <div className="bg-red-50 border border-red-100 rounded-2xl p-3 text-center">
            <p className="text-xl sm:text-2xl font-bold text-red-500">{users.length - ativos}</p>
            <p className="text-[11px] sm:text-xs text-red-500/70 font-medium mt-0.5">Inactivos</p>
          </div>
          <div className="bg-blue-50 border border-blue-100 rounded-2xl p-3 text-center sm:col-span-1 col-span-2">
            <p className="text-xl sm:text-2xl font-bold text-[#184d8a]">{users.length}</p>
            <p className="text-[11px] sm:text-xs text-[#184d8a]/70 font-medium mt-0.5">Total</p>
          </div>
        </div>
            ...
            {users.map((u) => (
              ...
              >
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                  <div className={`w-9 h-9 sm:w-10 sm:h-10 flex-shrink-0 rounded-xl flex items-center justify-center font-bold text-xs sm:text-sm ${
                    u.ativo ? "bg-[#184d8a]/10 text-[#184d8a]" : "bg-red-100 text-red-400"
                  }`}>
                    {u.nome.charAt(0).toUpperCase()}
                  </div>
                  <div className="min-w-0">
                    <p className="font-bold text-gray-700 text-xs sm:text-sm truncate">{u.nome}</p>
                    <p className="text-[10px] sm:text-xs text-gray-400 truncate">{u.email}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                  ...
                  <button
                    onClick={() => handleToggle(u)} disabled={toggling === u.id}
...
const TabSeguranca = () => {
  ...
  const forcaInfo = forca === null ? null
    : forca <= 1 ? { t: "Fraca",  cls: "bg-red-400",    txt: "text-red-600"    }
    : forca === 2 ? { t: "Média",  cls: "bg-yellow-400", txt: "text-yellow-600" }
    : forca === 3 ? { t: "Boa",   cls: "bg-blue-400",   txt: "text-blue-600"   }
  ...
  const handleChange = async () => {
    ...
    if (form.nova !== form.confirmar) { toast.error("As novas senhas não coincidem"); return; }
    if (form.nova.length < 8) { toast.error("A nova senha deve ter pelo menos 8 caracteres"); return; }
    setSaving(true);
    try {
      const token = getToken();
      ...
      if (!res.ok) { const e = await res.json(); throw new Error(e.erro || "Erro ao alterar"); }
    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="flex items-start gap-3 p-3 sm:p-4 bg-orange-50 border border-orange-100 rounded-2xl">
        ...
        <p className="text-[11px] sm:text-xs text-orange-700">
          Nunca partilhe a sua senha. A alteração entra em vigor imediatamente no sistema ClassCash.
        </p>
      </div>
            ...
            {key === "nova" && forcaInfo && (
              <div className="flex items-center gap-2 mt-1">
                <div className="flex gap-1">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className={`h-1.5 w-7 sm:w-8 rounded-full transition-all ${i <= (forca ?? 0) ? forcaInfo.cls : "bg-gray-200"}`} />
                  ))}
                </div>
                <span className={`text-[11px] font-bold ${forcaInfo.txt}`}>{forcaInfo.t}</span>
              </div>
            )}
      ...
      <div className="flex justify-end pt-1">
        <SaveBtn
          onClick={handleChange} loading={saving} label="Alterar Senha"
...
const TabNotificacoes = () => {
  const [prefs, setPrefs] = useState([
    { id: "propinas",    label: "Propinas em atraso",      desc: "Alerta quando uma propina ultrapassa a data limite",     ativo: true,  icon: <DollarSign size={14} className="text-red-500" /> },
    { id: "pagamentos",  label: "Novos pagamentos",         desc: "Notificação quando um pagamento é registado no sistema", ativo: true,  icon: <CheckCircle size={14} className="text-green-500" /> },
    { id: "reclamacoes", label: "Novas reclamações",        desc: "Alerta quando um encarregado submete uma reclamação",    ativo: false, icon: <Bell size={14} className="text-amber-500" /> },
    { id: "relatorios",  label: "Relatórios semanais",      desc: "Resumo automático enviado por email toda a semana",      ativo: true,  icon: <Settings size={14} className="text-blue-500" /> },
  ]);
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
        ...
        {prefs.map((p) => (
          <div
            ...
            className={`flex items-center justify-between p-3 sm:p-4 rounded-2xl border transition-all gap-3 ${
              p.ativo ? "bg-gray-50 border-gray-100" : "bg-white border-gray-100 opacity-60"
            }`}
          >
            <div className="flex items-start gap-3 flex-1 min-w-0">
              ...
              <div className="min-w-0">
                <p className="font-bold text-gray-700 text-xs sm:text-sm">{p.label}</p>
                <p className="text-[10px] sm:text-xs text-gray-400 mt-0.5 line-clamp-2">{p.desc}</p>
              </div>
            </div>
      ...
      <div className="flex justify-end pt-1">
        <SaveBtn onClick={() => toast.success("Preferências guardadas!")} label="Guardar Preferências" />
      </div>
    </div>
  );
};
          ...
          <div className="flex-1 overflow-hidden flex p-3 sm:p-4 lg:p-6 xl:p-8 gap-4 sm:gap-5 lg:gap-6">
            ...
            <aside className="hidden lg:flex w-56 xl:w-64 flex-col gap-1 flex-shrink-0 self-start">
              ...
              <div className="px-4 pb-3 mb-1">
                <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">Menu</p>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-2">
                {TABS.map((tab) => (
                  ...
                  >
                    <span className={`mt-0.5 shrink-0 ${activeTab === tab.id ? "text-white" : "text-gray-400 group-hover:text-[#184d8a]"}`}>
                      {tab.icon}
                    </span>
                    <div className="min-w-0">
                      <p className={`text-xs font-bold leading-tight ${activeTab === tab.id ? "text-white" : "text-gray-600 group-hover:text-gray-800"}`}>
                        {tab.label}
                      </p>
                      <p className={`text-[10px] mt-0.5 ${activeTab === tab.id ? "text-blue-200" : "text-gray-400"}`}>
                        {tab.desc}
                      </p>
                    </div>
                  </button>
              ...
              {user && (
                <div className="mt-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 bg-[#184d8a]/10 text-[#184d8a] rounded-xl flex items-center justify-center font-bold text-sm shrink-0">
                      {user.nome.charAt(0).toUpperCase()}
                    </div>
                    <div className="min-w-0">
                      <p className="font-bold text-gray-700 text-xs truncate">{user.nome}</p>
            ...
            <section className="flex-1 min-w-0 overflow-y-auto">
              ...
              <div className="mb-4 sm:mb-5 flex items-center gap-2">
                <div className={`w-8 h-8 rounded-xl bg-[#184d8a]/10 flex items-center justify-center text-[#184d8a]`}>
                  {activeTabInfo?.icon}
                </div>
                <div>
                  <p className="text-xs sm:text-sm font-bold text-gray-800">{activeTabInfo?.label}</p>
                  <p className="text-[10px] sm:text-xs text-gray-400">{activeTabInfo?.desc}</p>
                </div>
              </div>

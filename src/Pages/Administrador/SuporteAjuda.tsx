    ...
    mensagens: [
      {
        ...
        texto:
          "Bom dia, estou com dificuldade ao registar um pagamento. O sistema não está a aceitar o comprovativo em PDF.",
        hora: "14:30",
      },
      {
        id: 2,
        remetente: "secretaria",
        texto: "Já tentei várias vezes mas continua a dar erro.",
        hora: "14:32",
      },
    ],
  },
    ...
    mensagens: [
      {
        ...
        remetente: "secretaria",
        texto:
          "Boa tarde! Gostaria de saber como posso exportar o relatório mensal em Excel.",
        hora: "10:50",
      },
      {
        id: 2,
        remetente: "admin",
        texto:
          "Boa tarde, Maria! Para exportar em Excel, vá até Relatórios > clique em 'Exportar' > seleccione o formato Excel. Deve aparecer o download automaticamente.",
        hora: "11:00",
      },
      {
        id: 3,
        remetente: "secretaria",
        texto: "Obrigada, vou tentar agora mesmo.",
        hora: "11:15",
      },
    ],
  },
    ...
    mensagens: [
      {
        id: 1,
        remetente: "secretaria",
        texto: "Um estudante foi cadastrado mas não aparece na lista da turma.",
        hora: "09:00",
      },
      {
        id: 2,
        remetente: "admin",
        texto:
          "Verifiquei e o estudante estava associado a uma instituição diferente. Já corrigi.",
        hora: "09:20",
      },
      {
        id: 3,
        remetente: "secretaria",
        texto: "Problema resolvido, muito obrigada!",
        hora: "09:25",
      },
    ],
  },
    ...
    mensagens: [
      {
        ...
        remetente: "secretaria",
        texto:
          "O módulo de multas não está a carregar. A página fica a girar infinitamente.",
        hora: "08:45",
      },
    ],
  },
...
export default function SuporteAjuda() {
  ...
  const enviarMensagem = () => {
    ...
    setTicketActivo({
      ...ticketActivo,
      mensagens: [...ticketActivo.mensagens, nova],
      status: "Em andamento",
    });
    setNovaMensagem("");
  };
  ...
  const marcarResolvido = () => {
    const ticketsActualizados = tickets.map((t) =>
      t.id === ticketActivo.id
        ? { ...t, status: "Resolvido" as StatusTicket }
        : t,
    );
    setTickets(ticketsActualizados);
    ...
    <div className="flex bg-gray-50 font-sans min-h-screen">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between p-6 sticky top-0 z-40 bg-white/80 backdrop-blur border-b border-gray-100">
          <h1 className="text-xl font-bold text-[#184d8a]">Suporte e Ajuda</h1>
            ...
            <div className="w-80 shrink-0 bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col overflow-hidden">
              ...
              <div className="p-4 border-b border-gray-100">
                ...
                <div className="flex gap-1 flex-wrap">
                  {(
                    ["Todos", "Pendente", "Em andamento", "Resolvido"] as const
                  ).map((s) => (
                    <button
                      key={s}
                ...
                {filtrados.map((ticket) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-4 hover:bg-blue-50 transition-colors ${isActive ? "bg-blue-50 border-l-2 border-[#184d8a]" : ""}`}
                    >
                      <div className="flex items-start justify-between gap-2 mb-1">
                        ...
                        {ticket.naoLidas > 0 && (
                          <span className="w-5 h-5 bg-[#184d8a] text-white text-[10px] font-bold rounded-full flex items-center justify-center shrink-0">
                            {ticket.naoLidas}
                          </span>
                        )}
                      </div>
                      <p className="text-[10px] text-gray-500 mb-1.5">
                        {ticket.instituicao} · {ticket.utilizador}
                      </p>
                      ...
                      <div className="flex items-center justify-between">
                        <span
                          className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${STATUS_CONFIG[ticket.status].cor}`}
                        >
              ...
              <div className="p-4 border-b border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {ticketActivo.instituicao} · {ticketActivo.utilizador}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <span
                    className={`inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full ${STATUS_CONFIG[ticketActivo.status].cor}`}
                  >
                    {ticketActivo.status}
                  </span>
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <p
                            className={`text-[10px] mt-1 ${isAdmin ? "text-blue-200 text-right" : "text-gray-400"}`}
                          >
                            {isAdmin ? "Você" : ticketActivo.utilizador} ·{" "}
                            {msg.hora}
                          </p>
                        </div>
                      </div>
                ...
                {ticketActivo.status === "Resolvido" ? (
                  ...
                  <div className="flex gap-3">
                    <input
                      ...
                      className="flex-1 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#184d8a]/20 outline-none"
                    />
                    <button

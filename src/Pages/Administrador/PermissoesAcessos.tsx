...
import {
  AlertCircle, ChevronDown, Crown, Loader2,
  Lock, RefreshCw, Shield, Unlock, UserPlus, X,
} from "lucide-react";
...
function Toggle({
  ...
  const thumb = size === "sm" ? "w-3 h-3 top-0.5 left-0.5" : "w-4 h-4 top-1 left-1";
  ...
  return (
    ...
    >
      <span className={`pointer-events-none inline-block rounded-full bg-white transform transition-transform duration-200 ease-in-out absolute ${thumb} ${checked ? translate : "translate-x-0"}`}
...
// ─── Modal Novo Admin ─────────────────────────────────────────
function ModalNovoAdmin({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  const [form, setForm] = useState({ nome: "", email: "", senha: "", perfil: "Super Admin" });
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div className="flex justify-end gap-3 p-4 sm:p-5 bg-gray-50 border-t rounded-b-2xl">
          ...
          <button
            onClick={handleCriar} disabled={loading}
...
function UtilizadorCard({
  u, onToggleStatus, onTogglePlano,
}: {
  u: Utilizador; onToggleStatus: () => void; onTogglePlano: () => void;
}) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 sm:p-4 space-y-3">
      <div className="flex items-start justify-between gap-2">
        ...
        <span className={`flex-shrink-0 text-xs font-semibold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full ${STATUS_COR[u.status] ?? "bg-gray-100 text-gray-500"}`}>
          {u.status}
        </span>
      </div>
      <div className="flex flex-wrap gap-2">
        <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full ${BADGE[u.perfil]?.cor ?? "bg-gray-100 text-gray-600"}`}> {/* ✅ fallback */}
      ...
      <div className="flex items-center justify-between pt-2 border-t border-gray-50 gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Toggle checked={u.status === "Ativo"} onChange={onToggleStatus} size="sm" />
        </div>
      </div>
    </div>
  );
              ...
              {ABAS_SECRETARIA.map((aba) => (
                <div
                  ...
                  className={`flex items-center justify-between p-2.5 sm:p-3 rounded-xl border ${
                    aba.requerPlano ? "border-amber-200 bg-amber-50" : "border-gray-100 bg-gray-50"
                  }`}
                >
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ml-1 ${
                    aba.requerPlano ? "bg-amber-100 text-amber-700" : "bg-green-100 text-green-700"
                  }`}>

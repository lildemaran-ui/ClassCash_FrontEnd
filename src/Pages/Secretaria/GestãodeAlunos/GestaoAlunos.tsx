import {
  ArrowDown, ArrowUp, Bell, CircleUser, EyeIcon,
  PencilIcon, Plus, Search, Trash2,
} from "lucide-react";
...
export default function GestaoAlunos() {
  ...
  const colorsSit = (status: string) => {
    switch (status) {
      case "Aceite": return "text-green-500";
        ...
        <div className="px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          <section className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
            <div className="relative w-full sm:w-auto">
              ...
              <input type="search" placeholder="Procurar por nº processo ou nome..."
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div className="bg-white border p-6 rounded-3xl shadow-sm hover:shadow-md transition-shadow duration-500 flex flex-col">
              <h3 className="font-semibold text-gray-800 text-sm sm:text-lg mb-4">Distribuição de Ativos vs Inativos</h3>
              <div className="flex-grow flex items-center justify-center min-h-[250px] sm:min-h-[300px]">
                <ChartGestaoEstud />
              </div>
            </div>
            <div className="bg-white border p-6 rounded-3xl shadow-sm hover:shadow-md transition-shadow duration-500 flex flex-col">
              <h3 className="font-semibold text-gray-800 text-sm sm:text-lg mb-4">Evolução Mensal de Estudantes</h3>
              <div className="flex-grow flex items-center justify-center min-h-[250px] sm:min-h-[300px]">
                <ChartGestaoEstud2 />
              </div>
            </div>
              ...
              <table className="w-full text-center border-collapse min-w-[550px]">
                <thead>
                  <tr className="bg-[#184d8a]/70 text-white text-sm font-bold tracking-wide border-b border-gray-100">
                    ...
                    <th className="px-4 py-3 cursor-pointer" onClick={() => handleSort("nome_estudante")}>
                ...
                <tbody className="divide-y divide-gray-50">
                  {loading ? (
                    ...
                    <tr><td colSpan={5} className="py-12 text-center text-sm text-gray-400">Nenhum estudante encontrado</td></tr>
                  ) : (
                    filtrados.map((aluno) => (
                      <tr key={aluno.idestudante} className="hover:bg-[#184d8a]/5 transition-colors">
                        <td className="px-4 py-3 text-xs sm:text-sm font-mono text-gray-500">{aluno.num_processo}</td>
                        <td className="px-4 py-3 text-xs sm:text-sm font-medium text-gray-500">{aluno.nome_estudante}</td>
                        <td className="px-4 py-3 text-xs sm:text-sm text-gray-500">{aluno.classe ? `${aluno.classe}ª` : "—"}</td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex justify-center gap-2">
                            <div className="group relative">
                              ...
                              <span className="pointer-events-none absolute -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md bg-white border text-xs px-2 py-1 opacity-0 group-hover:opacity-100 transition-all z-10">Editar</span>
                            </div>
                            <div className="group relative">
                              <Link to="/DashboardEstud" className="p-2 bg-[#184d8a]/10 text-[#184d8a] rounded-lg hover:bg-[#184d8a] hover:text-white transition-all duration-300 shadow-sm block">
                                <EyeIcon size={16} />
                              </Link>
                              <span className="pointer-events-none absolute -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md bg-white border text-xs px-2 py-1 opacity-0 group-hover:opacity-100 transition-all z-10">Visualizar</span>
                            </div>
                            <div className="group relative">
                              <div onClick={() => handleDelete(aluno.idestudante)}
                              ...
                              <span className="pointer-events-none absolute -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md bg-white border text-xs px-2 py-1 opacity-0 group-hover:opacity-100 transition-all z-10">Excluir</span>
                            </div>
                          </div>
                        </td>
                      </tr>

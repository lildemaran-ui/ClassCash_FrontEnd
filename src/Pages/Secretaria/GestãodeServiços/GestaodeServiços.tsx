...
const ModalConfirmarExclusao = ({
  ...
  servico: Servico; onClose: () => void; onConfirm: () => void; loading: boolean;
}) => (
  <div
    ...
    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden">
      <div className="px-6 pt-8 pb-6 text-center">
        ...
        <p className="text-gray-500 text-sm">
          Tens a certeza que queres remover o serviço <strong className="text-gray-700">"{servico.servico}"</strong> da {servico.classe}ª classe?
        </p>
...
export default function GestaodeServiços() {
  ...
  const servicosFiltrados = servicos.filter(s =>
    !filtroServico || s.servico.toLowerCase().includes(filtroServico.toLowerCase())
  );
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans overflow-hidden custom_scroll">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <Header titulo="Gestão de Serviços" usuario={<Avatar name={user.nome} src={user.foto} size="sm" />} />
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
            {[
              ...
              { label: "Classes Cobertas", val: new Set(servicos.map(s => s.classe)).size },
              { label: "Valor Médio", val: servicos.length > 0
                ? (servicos.reduce((a, s) => a + Number(s.valorservico), 0) / servicos.length).toLocaleString("pt-AO", { style: "currency", currency: "AOA" })
                : "—"
              },
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-8">
            <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              ...
              <span className="text-xs text-gray-400 bg-white px-3 py-1 rounded-full border">{servicosFiltrados.length} serviços</span>
            </div>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-center border-collapse">
                ...
                <tbody className="divide-y divide-gray-50">
                  {loading ? (
                    <tr><td colSpan={5} className="py-14 text-center text-sm text-gray-400">
                    ...
                    <tr><td colSpan={5} className="py-14 text-center text-sm text-gray-400">Nenhum serviço encontrado</td></tr>
                  ) : servicosFiltrados.map((s) => (
                    <tr key={`${s.idservico}-${s.idclasse}`} className="hover:bg-[#184d8a]/3 transition-colors">
                      <td className="px-6 py-4 text-sm font-semibold text-gray-700 text-left">{s.servico}</td>
                      <td className="px-6 py-4 text-sm text-gray-500">{s.classe}ª Classe</td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex gap-2 justify-center">
                          <button
                            onClick={() => setModalServico({ open: true, servico: s })}
                          ...
                          <button
                            onClick={() => setModalExclusao({ open: true, servico: s })}
                ...
                <div key={`${s.idservico}-${s.idclasse}`} className="p-4 hover:bg-gray-50">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <p className="text-xs text-gray-400">{s.classe}ª Classe</p>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => setModalServico({ open: true, servico: s })} className="p-1.5 bg-[#184d8a]/10 text-[#184d8a] rounded-lg hover:bg-[#184d8a] hover:text-white transition-all">
                        <Pen size={14} />
                      </button>
                      <button onClick={() => setModalExclusao({ open: true, servico: s })} className="p-1.5 bg-red-50 text-red-400 rounded-lg hover:bg-red-500 hover:text-white transition-all">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </div>

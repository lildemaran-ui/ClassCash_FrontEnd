...
const ComprovatvivoViewer = ({ url }: { url: string }) => {
  ...
  const isTransacaoId = !isImage && !isPdf;
...
const ModalDetalhes = ({
  ...
  const handleValidar = async () => {
    setValidando(true);
    try {
      const res = await fetchComAuth(`${API}/gestaoPagamentos/${row.idpagamento}`, {
        method: "PUT",
...
const ModalEditar = ({
  ...
  row: PagamentoRow; onClose: () => void; onSave: () => void;
}) => {
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="bg-primary px-6 py-5 flex justify-between items-start">
          <div>
            ...
            <p className="text-blue-200 text-sm mt-0.5">{row.nome_estudante} · #{row.codigo}</p>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl bg-white/10 text-white hover:bg-white/20 transition-all">
            <X size={18} />
          </button>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setForm({ ...form, fk_pagamentos_servico: e.target.value })}
              className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#184d8a] cursor-pointer"
            >
...
const ModalAdicionar = ({
  ...
  const selecionarServico = (id: string) => {
    ...
    if (s) {
      ...
      setForm((f) => ({ ...f, valor: String(parseFloat(s.valorservico) * meses) }));
    }
  };
  ...
  const handleSave = async () => {
    ...
    const mesesReferencia = isPropina
      ? mesesDoAno
          .slice(mesesDoAno.indexOf(form.mesInicial), mesesDoAno.indexOf(form.mesFinal) + 1)
          .join(", ")
      : null;
        ...
        {loadingDados ? (
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  value={pesquisa}
                  onChange={(e) => { setPesquisa(e.target.value); setEstudanteSelecionado(null); setMostrarSugestoes(true); }}
              ...
              {estudanteSelecionado && (
                <div className="mt-2 flex items-center justify-between bg-primary/5 border border-[#184d8a]/20 rounded-xl px-3 py-2">
                  ...
                  <button
                    onClick={() => { setEstudanteSelecionado(null); setPesquisa(""); }}
            ...
            <div>
              ...
              >
                <option value="">
                  {!estudanteSelecionado ? "Seleccione primeiro um estudante..." : "Seleccionar serviço..."}
                </option>
                {servicos
            ...
            {/* Período de meses — propina mostra selectores, outros mostra input numérico */}
            {isPropina ? (
              <div className="flex flex-col gap-3 p-4 bg-blue-50 border border-blue-100 rounded-xl">
                  ...
                  <div className="flex-1">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const novoValor = servicoSelecionado ? parseFloat(servicoSelecionado.valorservico) * meses : 0;
                        setForm({ ...form, mesInicial: e.target.value, meses: String(meses), valor: String(novoValor) });
                      }}
                  ...
                  <div className="flex-1">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const meses = fim >= inicio ? fim - inicio + 1 : 1;
                        const novoValor = servicoSelecionado ? parseFloat(servicoSelecionado.valorservico) * meses : 0;
                        setForm({ ...form, mesFinal: e.target.value, meses: String(meses), valor: String(novoValor) });
                      }}
                      className="w-full border rounded-lg px-3 py-2 bg-white text-sm"
                    >
                      {mesesDoAno.map((m) => <option key={m} value={m}>{m}</option>)}
                ...
                <p className="text-xs text-blue-600 font-semibold">
                  {parseInt(form.meses) || 1} mês(es) selecionado(s) — Total:{" "}
              ...
              <div>
                ...
                <input
                  ...
                  value={form.meses}
                  onChange={(e) => {
                    const novosMeses = parseInt(e.target.value) || 1;
                    const novoValor = servicoSelecionado
                      ? parseFloat(servicoSelecionado.valorservico) * novosMeses
                      : parseFloat(form.valor) || 0;
                    setForm({ ...form, meses: e.target.value, valor: String(novoValor) });
                  }}
                  className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#184d8a] focus:ring-2 focus:ring-[#184d8a]/10 transition-all"
                />
              </div>
...
export default function GestaoPagamentos() {
  ...
  const [cards, setCards] = useState<Cards>({
    total_pagamentos: "0", confirmados: "0",
    pendentes: "0", atrasados: "0", cancelados: "0",
  });
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans overflow-hidden custom_scroll">
              ...
              <table className="w-full text-center border-collapse min-w-[700px]">
                ...
                <tbody className="divide-y divide-gray-50">
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={8} className="py-14 text-center text-sm text-gray-400">Nenhum pagamento encontrado</td>
                    </tr>
                  ) : (
                    tabelaFiltrada.map((row, i) => (
                      <tr
                        key={i}
                        className={`hover:bg-primary/3 transition-colors ${row.status.toLowerCase() === "pendente" && row.comprovativo_url ? "bg-amber-50/40" : ""}`}
                      >
      ...
      {selectedRow && (
        <ModalDetalhes row={selectedRow} onClose={() => setSelectedRow(null)} onValidar={carregar} />
      )}
      {modalEditar && (
        <ModalEditar row={modalEditar} onClose={() => setModalEditar(null)} onSave={carregar} />
      )}
      {showAdicionar && (
        <ModalAdicionar onClose={() => setShowAdicionar(false)} onSave={carregar} />
      )}
    </div>
  );
}
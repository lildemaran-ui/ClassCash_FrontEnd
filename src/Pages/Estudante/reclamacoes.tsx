...
import { MessageSquare, Send, Clock, CheckCircle, Pencil, X, Trash2 } from "lucide-react";
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div className="px-6 pb-6 flex gap-3">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex-1 py-3 rounded-xl font-bold text-white bg-primary hover:bg-[#1a5fad] transition-all flex items-center justify-center gap-2 disabled:opacity-60"
          >
            {saving
              ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> A guardar...</>
              : <><Send size={16} /> Guardar Alterações</>}
          </button>
        </div>
      </div>
    </div>
...
const ModalCancelar = ({ onClose, onConfirm }: { onClose: () => void; onConfirm: () => void }) => (
  ...
  >
    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-6 text-center">
      ...
      <p className="text-gray-500 text-sm mb-6">Tens a certeza que queres remover esta reclamação? Esta acção não pode ser revertida.</p>
    ...
    <div className="bg-white rounded-xl border border-gray-100 p-4 sm:p-5 flex flex-col gap-3 shadow-sm hover:shadow-md transition-shadow duration-300">
      ...
      {rec.resposta && (
        <div className="bg-blue-50 border border-blue-100 rounded-lg px-3 py-2">
          ...
          <p className="text-xs text-blue-800">{rec.resposta}</p>
        </div>
      )}

      {/* Acções — só disponíveis quando Pendente */}
      {podeEditar && (
        <div className="flex gap-2 justify-end pt-1">
          <button
            onClick={() => onEditar(rec)}
          ...
          <button
            onClick={() => onCancelar(rec.idreclamacao)}
...
export default function ReclamacaoGeral() {
  ...
  const handleEditar = async (id: number, titulo: string, descricao: string) => {
    try {
      ...
      if (response.ok) {
        toast.success("Reclamação actualizada com sucesso!");
        carregarReclamacoes();
      } else {
        const erro = await response.json();
        toast.error(`Erro: ${erro.erro || "Não foi possível actualizar."}`);
      }
    } catch {
      toast.error("Não foi possível conectar ao servidor.");
    }
  ...
  // Cancelar (eliminar) reclamação
  const handleCancelar = async () => {
    if (!modalCancelar) return;
    try {
      const response = await fetchComAuth(`http://localhost:5000/api/reclamacoes/${modalCancelar}`, {
        method: "DELETE",
      });
      if (response.ok) {
        toast.success("Reclamação cancelada.");
        setModalCancelar(null);
        carregarReclamacoes();
      } else {
        toast.error("Não foi possível cancelar a reclamação.");
      }
    } catch {
      toast.error("Não foi possível conectar ao servidor.");
    }
      ...
      <div className="flex flex-col xl:flex-row xl:items-start gap-6 sm:gap-8 w-full">
        ...
        <section className="flex flex-col gap-3 sm:gap-4 w-full xl:w-1/2 xl:sticky xl:top-6">
          <div className="flex items-center gap-2">
            ...
            <span className="bg-primary/10 text-[#184d8a] text-xs font-semibold px-2 py-0.5 rounded-full">
            ...
            <div className="flex justify-end">
              <button
                ...
                className={`w-full sm:w-auto flex items-center justify-center gap-2 text-white text-sm font-semibold px-6 py-2.5 rounded-lg transition-all duration-300 shadow-sm hover:shadow-md active:scale-95 ${
                  enviando ? "bg-gray-300 cursor-not-allowed" : "bg-primary hover:bg-[#1a5fa8]"
                }`}
              >
                <Send size={15} />

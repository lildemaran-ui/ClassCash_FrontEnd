...
import {
  ...
  Mail, Phone, XCircle, ChevronDown, Search,
} from "lucide-react";
    ...
    <div className="mt-3 overflow-hidden rounded-xl border border-blue-100 bg-gradient-to-br from-blue-50 to-white shadow-sm">
      <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-blue-100">
        ...
        <div className="p-5 space-y-3">
          <p className="text-[10px] font-bold text-blue-400 uppercase tracking-widest">Contacto</p>
          ...
          <p className="text-xs text-gray-500 leading-relaxed">{institution.address}</p>
        </div>

        {/* Col 2 — Informações */}
        <div className="p-5 space-y-4">
          <p className="text-[10px] font-bold text-blue-400 uppercase tracking-widest">Informações</p>
          <div>
            <p className="text-[10px] font-semibold text-gray-400 uppercase">Responsável</p>
            <p className="text-sm text-gray-800 mt-0.5">{institution.contactName}</p>
          </div>
          <div>
            <p className="text-[10px] font-semibold text-gray-400 uppercase">Membro desde</p>
        ...
        <div className="p-5 flex flex-col justify-between">
          <div>
            <p className="text-[10px] font-bold text-blue-400 uppercase tracking-widest mb-3">Status</p>
    ...
    <div className="min-h-screen bg-[#f0f5fa] flex flex-col font-sans">
      ...
      <section className="pt-36 pb-10 px-6 text-center">
        ...
        <h1 className="text-3xl md:text-4xl font-extrabold text-gray-800 max-w-2xl mx-auto leading-tight">
          ...
          <span className="text-[#268cff]"> registe‑se hoje</span>
        </h1>
        <p className="text-gray-500 text-sm mt-3 max-w-xl mx-auto">
          Confira as instituições parceiras da plataforma. Clique em
          <strong className="text-gray-700"> "Cadastrar-se"</strong> para criar a sua conta
          automaticamente vinculada à instituição escolhida.
        </p>
        ...
        <div className="relative max-w-md mx-auto mt-8">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 rounded-2xl border border-gray-200 bg-white shadow-sm text-sm focus:outline-none focus:ring-2 focus:ring-[#268cff]/30 transition"
          />
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between mb-4 px-1">
            <p className="text-xs text-gray-400 font-medium">
              {filtered.length} instituição{filtered.length !== 1 ? "s" : ""} encontrada{filtered.length !== 1 ? "s" : ""}
            </p>
          </div>
                  ...
                  <div key={inst.id} className="px-6 py-4">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="w-11 h-11 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-center shrink-0">
                        <Building2 className="w-5 h-5 text-[#268cff]" />
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-400 mt-0.5 truncate">{inst.address}</p>
                      </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        >
                          Detalhes
                          <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${expandedId === inst.id ? "rotate-180" : ""}`} />
                        </button>
                        ...
                        <button
                          ...
                          className={`text-xs font-bold px-4 py-1.5 rounded-lg transition-all duration-200 shadow-sm ${
                            inst.status === "Ativo"
                              ? "bg-[#268cff] text-white hover:bg-blue-600 hover:shadow-md active:scale-95"
                    ...
                    {/* Detalhes expandidos */}
                    {expandedId === inst.id && <ExpandedDetails institution={inst} />}
                  </div>
                ))}
              </div>
            )}
